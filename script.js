...
async function loadApplication(user) {
  ...
  const { data: profile, error } = await supabaseClient
    .from("profiles")
    .select("id, full_name, email, role, school_id, schools(id, school_name)")
    .eq("id", user.id)
    .single();

  if (error || !profile) {
    showMessage("loginMessage", "Your profile has not been configured. Contact an administrator.", true);
